...
public class RHPAMController {
    ...
    private AbstractKieServerConnector createBusinessSvcConnector(String Authorization, String businessActivity) {
        ...
        AbstractKieServerConnector connector = new AbstractKieServerConnector(Authorization, helper.getBusinessAliasServiceURLMappings().get(helper.getBusinessActivityAliasMappings().get(businessActivity)));
	...
	@ResponseBody
    public ResponseEntity<ExpenseValidationResponse> expenseValidation(@RequestBody ExpenseValidations body, @RequestHeader String Authorization) {
        ...
        AbstractKieServerConnector connector = createBusinessSvcConnector(Authorization, body.getDecision());
	...
	@ResponseBody
    public ResponseEntity<List<ProcessInstance>> processInstances(@PathVariable("processId") String processId, @RequestHeader String Authorization) {
	...
	public ResponseEntity<List<TaskSummary>> searchTasks(@RequestBody TaskSearch body, @PathVariable("userId") String userId
        ...
        if (body.getProcessId()== null){
            ...
            for (Map.Entry<String, String> entry : knownRhpamServices.entrySet()) {
                ...
                connectors.put(alias, new AbstractKieServerConnector(Authorization, serviceUrl));
            }
            ...
            connectors.put(processId, createBusinessSvcConnector(Authorization, body.getProcessId()));
        }
  ...
  // ??? Add Human Task Comment (Used for history and rejection)  (TBD)
   ...
   // Search for a history on a specific business process based Task and Comment Tables and TaskVariableImpl for variable name pApprovalStatus (see Business Process documentation 2.1.5  Expenses Approvals   -  Tasks and Groups/Roles (TBD If custom query)
	...
	@ResponseBody
	public String taskApprovalsHistory(@PathVariable("pid") String pid, @RequestHeader String Authorization) {
...
//}
	...
	@ResponseBody
	public String autoCompleteTask(@PathVariable("taskId") String taskId,  @PathVariable("actor") String actor, @RequestHeader String Authorization) {
        LOGGER.debug("autoCompleteTask["+taskId+"] for actior ["+actor+"]");
